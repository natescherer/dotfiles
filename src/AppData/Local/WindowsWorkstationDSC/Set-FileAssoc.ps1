# Vendored verbatim (content unmodified below the next blank line) from
# https://github.com/default-username-was-already-taken/set-fileassoc (Set-FileAssoc.ps1, master
# branch, commit as of 2020-09-26 -- the repo's only commits), fetched 2026-08-02. Public domain
# (Unlicense, per the repo's GitHub license metadata) -- reviewed in full before vendoring: no
# network calls, no obfuscation, a documented/public reimplementation (in inline C#, to sidestep
# PowerShell integer-overflow pitfalls) of the same reverse-engineered `UserChoice` hash algorithm
# SetUserFTA uses, scoped to writing exactly the one target extension's UserChoice registry value
# for the current user -- no compiled binary, no external download at runtime, so nothing here
# needs re-fetching or re-trusting on every `chezmoi apply`. Picked over SetUserFTA specifically to
# avoid depending on a downloaded, unauditable third-party .exe; picked over Windows' own
# DISM/Group-Policy DefaultAssociationsConfiguration mechanism because that one only takes effect
# for brand-new user profiles (or requires a full export-and-splice of every existing association
