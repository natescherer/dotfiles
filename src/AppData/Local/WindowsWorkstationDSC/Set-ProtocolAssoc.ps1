# Modified copy of Set-FileAssoc.ps1 (same directory) -- adapted to set the per-user UserChoice
# registry value for URL protocol associations (http, https) instead of file extensions, so a
# browser can be set as the default handler for web links without admin rights, the same way
# Set-FileAssoc.ps1 does for individual file types. See that file's header for the full
# provenance/license/no-admin-required rationale (source:
...
<#
    .SYNOPSIS
        ...
        Sets Windows URL protocol associations (e.g. the default browser) on a per-user basis, bypassing the built-in protection.

    .DESCRIPTION

        This script allows a user or an IT administrator to change a user's default handler for a URL protocol (such as http/https, i.e. the default browser) in Windows 10.

        Per-user protocol associations in newer versions of Windows are normally protected from an unauthorized change,
    ...
    .PARAMETER Protocol
        ...
        Specifies a URL protocol/scheme that an association will be set for.

        Example: http

    .PARAMETER ProgID

        Specifies the ProgID - an application identifier for the protocol handler.
    ...
    .EXAMPLE
        ...
        C:\> .\Set-ProtocolAssoc.ps1 -Protocol http -ProgID ChromeHTML -AllUsers

    .EXAMPLE

        C:\> .\Set-ProtocolAssoc.ps1 -Protocol https -ProgID ChromeHTML -Users user1, user2
...
Param (
    [Parameter(Mandatory, Position=0, HelpMessage="URL protocol", ParameterSetName="CurrentUser")]
    [Parameter(Mandatory, Position=0, HelpMessage="URL protocol", ParameterSetName="AllUsers")]
    [Parameter(Mandatory, Position=0, HelpMessage="URL protocol", ParameterSetName="SpecificUsers")]
...
function Get-InputStringForUser($User) {
    return ("{0}{1}{2}{3}{4}" -f $Protocol, $User.SID, $ProgID, $User.WriteTime,
...

{
    ...
    foreach ($User in $EnumeratedUsers) {
        ...
        Write-Host "Protocol association set for user `"$($User.Name)`": $Protocol => $ProgID (hash `"$Hash`")"
